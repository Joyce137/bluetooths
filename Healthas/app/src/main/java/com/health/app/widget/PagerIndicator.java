...
import com.mslibs.widget.CLinearLayout;
...
public class PagerIndicator extends CLinearLayout {
	...
	public void setPagerCount(int count) {
		...
		for (int i = 0; i < mPagerCount; i++) {
			...
			LayoutParams params = new LayoutParams((int) (8 * metrics.density),
					(int) (8 * metrics.density));
			params.setMargins((int) (4 * metrics.density),
					(int) (6 * metrics.density), (int) (4 * metrics.density),
					(int) (6 * metrics.density));
	...
	public void setPagerIndex(int index) {
		...
		if (index < getChildCount()) {
			...
			if (currentPoint != null) {
				if (isGray) {
					currentPoint
							.setBackgroundResource(R.drawable.rounded8_gray_n);
				} else {
