package com.health.app.database.support;
...
import com.health.app.database.DBManagerForExist;
import com.health.app.database.DBManagerForNew;
...
public class DaoSupportImpl<DBEntity> implements IDaoSupport<DBEntity> {
    ...
    public void closeDatabase(){
        if(dbManagerForExist != null)
            dbManagerForExist.closeDatabase();
        if(db.isOpen()){
            db.close();
        }
    }
    ...
    public ArrayList<DBEntity> executeSql(String[] columns,String sqlStr) {
        ...
        sqlStr = "select *" + " from " + DBConstants.TABLE_BLE + sqlStr;
        db.beginTransaction();
